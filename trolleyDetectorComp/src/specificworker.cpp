   ...
   // test();
    ...
    try {
        ...
        { 
            ...
            std::vector<std::pair<float, cv::Point2f>> coords_info = realsensem.getDistanceAndCoords(selection, color_frame, depth_frame, objects);
          
            
            unsigned int npeople_aux = realsensem.getNumberOfPeople(objects);
            if(npeople != npeople_aux)
            {
                npeople = npeople_aux;
                std::cout << "Personas: " << npeople << std::endl;
                try {
                    persontoagent_proxy->peopleDetected(npeople);
                } catch(...){}
            }
            if(coords_info.size() >= 2)
            {
                if(coords_info.size() == 4) //si ve menos ruedas el algoritmo puede no funcionar bien. TODO
                {
                    realsensem.boundedRectangle(coords_info);
                }
                std::pair<float, cv::Point2f> tag_pose;
                if(realsensem.getTagPose(tag_pose, coords_info))
                {
                    double x, z, angle;
                    tag_pose_t tag_matrix = realsensem.createTagMatrix(tag_pose);
                    
                    tag_pose_t pose_fr = realsensem.getGoalFromPose(tag_matrix, 3.14, 0, 1.0);
                    x = pose_fr.t[0];
                    z = pose_fr.t[2];
                    ...
                    sendTrolleyEstimationToAgent(x, z, angle, realsensem.rectangleEstimation(coords_info));
                    
                    pose_fr = realsensem.getGoalFromPose(tag_matrix, 0, 0, -0.35);
                    x = pose_fr.t[0];
                    z = pose_fr.t[2];
                    angle = atan2 (pose_fr.r[2], pose_fr.r[0]);
                    ...
                    pose_fr = realsensem.getGoalFromPose(tag_matrix, 0, 0, 1);
                    x = pose_fr.t[0];
                    z = pose_fr.t[2];
                    angle = atan2 (pose_fr.r[2], pose_fr.r[0]);
...
void SpecificWorker::processLaserData()
{
    ...
    */
    ...
    if(laser_data.size())
    {
        ...
        tag_pose_t pose_from_laser = calculaObjetivoSA3IR(tag_matrix, 0.0, 0, 0.60);

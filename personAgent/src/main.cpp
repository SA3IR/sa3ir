...
/** \mainpage RoboComp::personAgent
 *
 * \section intro_sec Introduction
 *
 * The personAgent component...
 *
 ...
 *
 * Just: "${PATH_TO_BINARY}/personAgent --Ice.Config=${PATH_TO_CONFIG_FILE}"
 *
...
class personAgent : public RoboComp::Application
{
public:
	personAgent (QString prfx) { prefix = prfx.toStdString(); }
private:
	void initialize();
	std::string prefix;
	MapPrx mprx;
...
int ::personAgent::run(int argc, char* argv[])
{
	...
	try
	{
		...
		// Server adapter creation and publication
		if (not GenericMonitor::configGetString(communicator(), prefix, "PersonToAgent.Endpoints", tmp, ""))
		{
			cout << "[" << PROGRAM_NAME << "]: Can't read configuration for proxy PersonToAgent";
		}
		Ice::ObjectAdapterPtr adapterPersonToAgent = communicator()->createObjectAdapterWithEndpoints("PersonToAgent", tmp);
		PersonToAgentI *persontoagent = new PersonToAgentI(worker);
		adapterPersonToAgent->add(persontoagent, Ice::stringToIdentity("persontoagent"));
		adapterPersonToAgent->activate();
		cout << "[" << PROGRAM_NAME << "]: PersonToAgent adapter created in port " << tmp << endl;

...
int ::navigationagent::run(int argc, char* argv[])
{
	...
	try
	{
		...
		// Server adapter creation and publication
		if (not GenericMonitor::configGetString(communicator(), prefix, "LocalNavigatorReportState.Endpoints", tmp, ""))
		{
			cout << "[" << PROGRAM_NAME << "]: Can't read configuration for proxy LocalNavigatorReportState";
		}
		Ice::ObjectAdapterPtr adapterLocalNavigatorReportState = communicator()->createObjectAdapterWithEndpoints("LocalNavigatorReportState", tmp);
		LocalNavigatorReportStateI *localnavigatorreportstate = new LocalNavigatorReportStateI(worker);
		adapterLocalNavigatorReportState->add(localnavigatorreportstate, Ice::stringToIdentity("localnavigatorreportstate"));
		adapterLocalNavigatorReportState->activate();
		cout << "[" << PROGRAM_NAME << "]: LocalNavigatorReportState adapter created in port " << tmp << endl;
